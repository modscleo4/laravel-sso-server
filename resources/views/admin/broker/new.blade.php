...
@section('title', 'Novo Broker')

@section('content_header')
    <h1>Adicionar novo Broker</h1>
@stop

@section('content')
    <form class="form-horizontal" action="{{ route('admin.broker.store') }}" method="post">
        @csrf
                        ...
                        <select class="form-control selection" id="inputRoles" name="roles[]" multiple>
                            ...
                            @foreach($roles as $role)
                                ...
                                <option value="{{ $role->id }}"
                                    {{ (old('roles') ? in_array($role->id, old('roles')) : false) ? 'selected' : '' }}>
                                    {{ $role->friendly_name }}
                                </option>
